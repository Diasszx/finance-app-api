...
import { internalServerError, notFound } from "../utils/http-response.js";
...
export class GetTransactionsByUserIdController {
  constructor(private readonly getTransactionByUserId: GetTransactionByIdServiceInterface) {}
  async execute(req: TypedRequestQuery<GetTransactionByIdQueryDTO>, res: Response) {
    try {
      const { userId } = req.query;
      const userIdIsValid = getTransactionByUserIdQuerySchema.parse(userId);
      const user = this.getTransactionByUserId.execute(userId);
    } catch (error) {
      console.log(error);
